...
description: >
  Use this skill when the user works with Doosan Robotics robots — DRL Python API, DRFL C++ API,
  DRFLEx, the doosan-robot2 ROS 2 stack, or anything in `~/cobot_ws/src/doosan-robot2/`. Trigger
  on motion functions (movej, movel, movec, moveb, movesj, movesx, movejx, move_spiral,
  move_periodic, jog, jog_multi, alter_motion), servo/speed streaming (servoj, servol, speedj,
  speedl, servoj_rt_stream, servol_rt_stream, torque_rt_stream), force/compliance
  (task_compliance_ctrl, set_desired_force, set_stiffnessx, parallel_axis, align_axis), real-time
  control (read_data_rt, write_data_rt, connect_rt_control, start_rt_control, set_velj_rt),
  system/safety (set_robot_mode, get_robot_state, set_robot_control, change_collision_sensitivity,
  set_singularity_handling), I/O (set_digital_output, get_digital_input, modbus signals), TCP/tool
  config (add_tcp, add_tool), DRL programs (drl_start, drl_stop, drl_pause), kinematics
  (trans, fkin, ikin, get_solution_space), 161 dsr_msgs2 services and 18 stream topics,
  the bringup process (dsr_bringup2 launch with mode:=virtual/real/sim), the controller stack
  (dsr_controller2, dsr_hardware2, port 12345/12347), the Python wrapper (DSR_ROBOT2.py,
  DR_init.__dsr__node, g_node, _srv_name_prefix), MoveIt2 m0609 config, URDF/xacro for m0609,
  the dev-docs at `/home/hoon/Documents/services/` and `/home/hoon/Documents/topics/`, RT topic
  publishing (`/rt_topic/<key>`, use_rt_topic_pub, rt_timer_ms), action clients (MovejH2r,
  MovelH2r, JogH2r), DRFL callbacks (OnMonitoringDataEx, OnMonitoringCtrlIO, 50ms callback
  constraint), DRCF firmware (M2.12+, virtual emulator on port 12345), and the two-process
  architecture (bringup + behavior). Also activate on Korean phrases: "두산 로봇", "두산 로보틱스",
  "DRFL", "DRL", "CDRFLEx", "두산 협동로봇", "도산로봇", "ROS2 두산", "DSR_ROBOT2", "doosan-robot2",
  "m0609", "cobot_ws", "두산 모션", "두산 좌표계", "두산 힘제어", "두산 RT 제어", "두산 IO",
  "두산 launch", "dsr_bringup2", "dsr_controller2", "dsr_msgs2". Trigger even when the user does
  not explicitly say "Doosan" — if the question involves topics like motion services on a
  cooperative arm, port 12345/12347, the `dsr01` namespace, or files in `~/cobot_ws/src/doosan-robot2/`,
  this skill applies. For general ROS 2 patterns (QoS, lifecycle, executors, rqt, RViz plugins) that
  are vendor-neutral, the `ros2-architect` skill is complementary; both can be active at once.
...
## 결정 라우터 (Decision Router)

이 스킬은 **두 종류의 자료**를 갖고 있다. 작업 시작 전 어느 쪽을 읽을지 먼저 결정한다.

| 작업 카테고리 | 1차 자료 |
|---|---|
| **함수 시그니처/인자/예제** (예: "movej 인자 알려줘") | 이 SKILL.md 본문 (PART 1·2·3) |
| **dsr_msgs2 서비스/토픽 정확한 사양** (143 srv + 18 topic) | `/home/hoon/Documents/services/<카테고리>/<name>.md` 또는 `/home/hoon/Documents/topics/<name>.md` 직접 열람 — 카탈로그는 `references/dev-docs-index.md` |
| **doosan-robot2 코드베이스 구조** (왜 이렇게 동작하는지) | `references/codebase-architecture.md` |
| **`/rt_topic/<key>` / `*_rt_stream` 패턴** (RT 데이터 push/pull) | `references/rt-topics.md` |
| **virtual / real / sim 모드, launch 구성** | `references/launch-and-modes.md` |
| **multi-robot, deadlock, 50ms 콜백, 펌웨어 등 함정** | `references/multi-robot-and-quirks.md` |
| **일반 ROS 2 패턴** (QoS, lifecycle, executor, rqt, rviz plugin) | 별도 스킬 `ros2-architect` 사용 |

**원칙**: 코드 작성 전에 `/home/hoon/Documents/{services,topics}/` 의 해당 md 를 먼저 열람한
다. 시그니처와 비고가 거기에 있다. 그 다음 이 SKILL.md 의 패턴 가이드와 `references/` 의 코드
베이스 분석을 합쳐서 작성한다.
...
`~/cobot_ws/src/doosan-robot2/` 의 실제 코드를 읽고 확인한 사실들. 이걸 모르면 이상한 코드를
쓰게 된다.

1. **dsr_controller2 는 표준 ros2_control 컨트롤러가 아니다.** `command_interface_configuration()`
   이 빈 벡터를 반환한다. 모션은 service-driven 이지 update loop driven 이 아니다. 따라서 표준
   `JointTrajectoryController` 의 거동을 기대하지 마라 (자세히: `references/codebase-architecture.md` §2).
2. **dsr_hardware2 는 read()/write() 를 오버라이드하지 않는다.** position/velocity 인터페이스만
   export, effort 는 TODO. force control 이 필요하면 hardware_interface 가 아니라 force services
   (`task_compliance_ctrl`, `set_desired_force`) 로 직접 호출.
3. **mode 는 부팅 시 결정되고 바뀌지 않는다.** virtual ↔ real 동적 전환 불가. RT 채널 (port
   12347) 은 real 모드에서만 활성화. virtual 에서 `read_data_rt` 같은 RT 서비스 호출하면 무응답.
4. **DRCF 펌웨어 ≥ M2.12** 강제. 미달이면 connect 자체 실패. 다운그레이드 워크어라운드 없음.
5. **DRFL 콜백은 50ms 안에 반환해야 한다.** 무거운 일은 큐로 넘기고 별 스레드에서 처리.
6. **DSR_ROBOT2.py 는 multi-robot 미지원.** `_srv_name_prefix=''` 로 namespace 가 `dsr01` 에 박
   혀있다. 두 로봇 다루려면 raw rclpy 또는 프로세스 분리.
7. **161개 인터페이스 (143 srv + 18 msg + 3 action) 는 모두 `/home/hoon/Documents/{services,topics}/`
   에 1차 자료로 정리돼 있다.** 새 코드 작성 시 이 docs 를 먼저 본다. SKILL.md 본문은 보조.
8. **DSR g_node 와 같은 executor 에 sensor topic subscription 을 두면 starve 된다.** DSR 의 모든
   service 호출이 `spin_until_future_complete(g_node, future)` 로 글로벌 executor 의 spin lock 을
   점유하기 때문. 100Hz topic 이 0.3Hz 만 처리되는 catastrophic data loss 발생. 해결: subscription
   전용 별도 노드 + 별도 `SingleThreadedExecutor` + 별도 thread (자세히: `multi-robot-and-quirks.md` §11).

이 8가지가 두산 ROS2 코드의 "왜 이렇게 동작하는지" 의 80% 를 설명한다. 더 깊은 분석은
`references/codebase-architecture.md` 의 10개 섹션 + `multi-robot-and-quirks.md` 의 11개 함정 참고.
...
`$ARGUMENTS`를 파싱하여:
- **함수명** (예: `movej`, `movel`) → 해당 함수 전체 시그니처·파라미터·예제 + (필요시) 해당
  서비스의 dev-docs md 파일 읽기 권장
- **`code:<설명>`** → 설명에 맞는 완전한 코드 생성. 코드 작성 전에 사용할 모든 service/topic
  의 dev-docs md 를 먼저 열람.
- **`workflow:<주제>`** → 주제별 단계별 절차 안내. virtual/real 모드 차이, 라이프사이클 순서,
  복구 시퀀스 등은 `references/launch-and-modes.md` 참고.
- **`debug`** → 사용자 코드 분석 후 문제점 및 수정 방법 제시. `references/multi-robot-and-quirks.md`
  의 알려진 함정 11가지를 우선 점검.
- **인자 없음** → 위 결정 라우터 + 전체 목차 출력.
...
이 SKILL.md 가 함수·패턴 가이드라면, 아래 5개 reference 파일은 **두산 ROS2 코드베이스 자체에
대한 분석**과 **dev-docs 활용법**을 다룬다. 비-자명한 작업 (예: RT 패턴 설계, multi-robot, 디
버깅) 시 직접 열람.

| 파일 | 다루는 내용 |
|---|---|
| `references/codebase-architecture.md` | dsr_controller2/dsr_hardware2/DSR_ROBOT2.py 가 실제로 어떻게 동작하는지 — 4계층 구조, 161개 인터페이스 카탈로그, 알려진 TODO |
| `references/dev-docs-index.md` | `/home/hoon/Documents/services/` 와 `/topics/` 의 사용법 — 카테고리별 핵심 서비스 표, md 파일 5섹션 표준, 빠른 grep 패턴 |
| `references/rt-topics.md` | `/rt_topic/<key>` (읽기) 와 `*_rt_stream` (쓰기) 패밀리의 차이, 활성화 yaml, 디버깅 6단계, alter_motion 패턴 |
| `references/launch-and-modes.md` | 5개 launch 파일과 virtual/real/sim 모드 분기, URDF/xacro 분기 메커니즘, CI 패턴, bringup vs behavior 분리 |
| `references/multi-robot-and-quirks.md` | 11가지 알려진 함정과 회피 — multi-robot 미지원, deadlock, 50ms 콜백, M2.12 강제, force/effort 미연결 등 |

추가로 **vendor-neutral ROS 2 패턴** (QoS, lifecycle 일반론, executor, rqt/rviz plugin) 은 별도
...
1. **사용할 service/topic 이름 식별** → `references/dev-docs-index.md` §카테고리별 표 또는
   `/home/hoon/Documents/services/<cat>/<name>.md` 직접 grep.
2. **md 파일의 5섹션 모두 읽기** — 특히 §2 Python 호출 패턴 + §5 비고.
3. **Python 래퍼가 있는지 확인** — 없으면 raw rclpy 패턴 사용 (`references/codebase-architecture.md`
   §6).
4. **모드 결정** — virtual 로 충분한지, real 모드 + RT 가 필요한지 (`references/launch-and-modes.md`).
5. **RT 가 필요하면** `references/rt-topics.md` 의 활성화 yaml 부터 점검.
6. **두산 함정 사전 체크** → `references/multi-robot-and-quirks.md` 11가지 항목 중 해당사항 점검.
7. **CLAUDE.md 의 dsr-skill 섹션** 도 함께 읽기 — 프로세스 분리, 초기화 순서, 정지·복구 시퀀스
   같은 운영 규칙은 거기에 정리돼 있다.
8. **코드 작성 + DSR_ROBOT2 import 순서 준수** — `DR_init.__dsr__node = node` → `from DSR_ROBOT2
   import ...` → 사용. 거꾸로 하면 g_node 가 None 으로 고정된다.

이 8단계가 두산 ROS2 코드 작성의 표준 워크플로우다. 이걸 따라 작성한 코드는 prod 에 가까운 품
질이 나온다. 무시하면 거의 항상 어딘가에서 함정에 빠진다.
